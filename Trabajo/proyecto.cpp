...
#include <cstdlib>
#include <cctype>
#include "../Usos/Bizi-usos.h"
#include "../Usuarios/Bizi-usuarios.h"
#include "../Estaciones/Bizi-estacion.h"
...
int main(){
	...
	char ficheroUsos[MAX_LONG_NOMBRE_FICHERO] = "usos-";
	// fichero de usuarios del sistema bizi de Zaragoza 
	char ficheroUsuarios[MAX_LONG_NOMBRE_FICHERO] = "usuarios-";
	// fichero de estaciones del sistema bizi de Zaragoza
	char ficheroEstaciones[MAX_LONG_NOMBRE_FICHERO] ="estaciones-";
	...
	int usosNormales, usosCirculares, numUsuarios;
	...
	cout << " El fichero " << ficheroUsuarios << " existe y tiene " << usosNormales + usosCirculares << " utilizaciones " << endl;
	cout << " El fichero " << ficheroUsos << " existe y tiene " << numUsuarios << " usuarios " << endl;
	...
	while (strcmp(ordenMod, "FIN") != 0){
		...
		if (indiceEspacio != ERROR_EN_BUSQUEDA){
			...
			// copia hasta el espacio
			strcpy(valor,"\0");
			strncpy(ordenMod, orden, indiceEspacio - 1);
			copiarFragmentoCadena(orden, valor, indiceEspacio + 1);
		}
		else {
			// tratar orden sin espacios en blanco
